...
function propanel_ariatheme_siteoptions_add_admin() {

    global $query_string;
   
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'ariatheme_siteoptions' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  get_option('of_template'); 
			propanel_of_reset_options($options,'ariatheme_siteoptions');
			header("Location: admin.php?page=ariatheme_siteoptions&reset=true");
			die;
		}
    }
		
    $tt_page = add_theme_page('تنظیمات پوسته', 'تنظیمات پوسته', 'edit_theme_options', 'ariatheme_siteoptions','propanel_ariatheme_siteoptions_options_page');
	add_action("admin_print_scripts-$tt_page", 'propanel_of_load_only');
	add_action("admin_print_styles-$tt_page",'propanel_of_style_only');
} 

add_action('admin_menu', 'propanel_ariatheme_siteoptions_add_admin');
...
function propanel_ariatheme_siteoptions_options_page(){
    $options =  get_option('of_template');
?>

<div class="wrap" id="truethemes_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">تنظیمات ذخیره شدند</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">تنظیمات ریست شدند</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    ...
    <?php 
        ...
        $return = propanel_ariatheme_siteoptions_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
    ...
    <input type="submit" value="ذخیره تغییرات" class="button-primary" />
  </form>
  <form action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ) ?>" method="post" style="display:inline" id="ofform-reset">
    <span class="submit-footer-reset">
    <input name="reset" type="submit" value="بازگشت به حالت اولیه" class="button submit-button reset-button" onclick="return confirm('اخطار : همه تنظیمات به حالت اولیه باز میگردند، آیا مایل به انجام این کار هستید؟');" />
...
function propanel_of_style_only() {
	wp_enqueue_style('admin-style', get_template_directory_uri().'/admin/admin-style.css?v=1.01');
  ...
  jQuery("a[title=submit]").click( function(){
    ...
    jQuery(this).parents("form").submit();
			...
			jQuery('#ofform').submit(function(){
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'ariatheme_siteoptions'){ ?>
						type: 'options',
						<?php } ?>
...
function propanel_ariatheme_siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			...
			if ( $std != "") { $val = $std; }
			$output .= '<input dir="rtl" class="of-input fullwidth" name="'. $value['id'] .'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. $val .'" />';
		break;
		...
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				$output .= '<textarea dir="rtl" class="of-input fullwidth" name="'. $value['id'] .'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.$ta_value.'</textarea>';
		...
		case "upload":
			...
			$output .= propanel_ariatheme_siteoptions_uploader_function($value['id'],$value['std'],null);
		...
		case "upload_min":
			...
			$output .= propanel_ariatheme_siteoptions_uploader_function($value['id'],$value['std'],'min');
		...
		case "color":
			...
			$output .= '<input dir="rtl" class="of-color colorinput" name="'. $value['id'] .'" id="'. $value['id'] .'" type="text" value="'. $val .'" />';
		break;   
		...
		case "heading":
			...
			$menu .= '<li><a title="'.  $value['fname'] .'" href="#'.  $jquery_click_hook  .'">'.  $value['fname'] .'</a></li>';
...
function propanel_ariatheme_siteoptions_uploader_function($id,$std,$mod){
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">آپلود تصویر</span>';
	
	if(!empty($upload)) {$hide = '';} else { $hide = 'hide';}
	
	$uploader .= '<span class="button image_reset_button '. $hide.'" id="reset_'. $id .'" title="' . $id . '">حذف کردن</span>';
	$uploader .='</div>' . "\n";
